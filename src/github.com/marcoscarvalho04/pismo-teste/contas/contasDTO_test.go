...
func TestSucessoConvertDTO(t *testing.T) {
	...
	if contaDTO.ContaId != conta.ContaId {
		t.Errorf(ERRO_CONVERT_DTO, "contaId", strconv.Itoa(conta.ContaId), strconv.Itoa(contaDTO.ContaId))
	}
...
func TestSuccessParseDTO(t *testing.T) {
	...
	if result.ContaId != 0 {
		t.Errorf("ContaId invalido, esperado: 0, resultado: " + strconv.Itoa(result.ContaId))
	}
	if result.Saldo != 0 {
